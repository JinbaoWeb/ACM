...
const int INF = (1<<30);
int pre[MAXN],ch[MAXN][2],key[MAXN],size[MAXN];	//pre表示父节点，ch[0]表示左子节点,ch[1]表示右子节点，key表示节点的值，size表示区间的大小
int root,tot1;	//root表示根节点，tot1表示
int sum[MAXN],rev[MAXN],same[MAXN];	//rev、same都是lazy标记
int lx[MAXN],rx[MAXN],mx[MAXN];	//左右子节点的值，
...
//以上是debug部分**************************************
void NewNode(int &r,int father,int k)	// r表示插入的位置，k表示插入元素的值，father表示要插入的元素的父节点
{
